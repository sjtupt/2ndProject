...
// And TestDeviceDriver5's DriverBinding Version = 0x10, which is lower than
...
} DEVICE_DRIVER_5_PRIVATE_DATA;

#define DEVICE_DRIVER_5_PRIVATE_DATA_FROM_DRIVER_BINDING(a) \
 _CR(a, DEVICE_DRIVER_5_PRIVATE_DATA, DriverBinding)

DEVICE_DRIVER_5_PRIVATE_DATA          *mPrivateData;

EFI_STATUS
InitializeDeviceDriver5 (
  IN EFI_HANDLE           ImageHandle,
  IN EFI_SYSTEM_TABLE     *SystemTable
  );

EFI_STATUS
DeviceDriver5BindingSupported (
...
//
EFI_DRIVER_ENTRY_POINT(InitializeDeviceDriver5)
  ...
  )
{
  ...
  Status = gtBS->AllocatePool (
                        EfiBootServicesData,
                        sizeof (DEVICE_DRIVER_5_PRIVATE_DATA),
                        (VOID**)&mPrivateData
                        );
  if (EFI_ERROR(Status)) {
    return Status;
  }
  gtBS->SetMem (mPrivateData,sizeof (DEVICE_DRIVER_5_PRIVATE_DATA),0);
  ...
  LoadedImageInfoPtr->Unload = DeviceDriver5Unload;
  ...
  )
{
  ...
  DEVICE_DRIVER_5_PRIVATE_DATA          *PrivateData;

  PrivateData = DEVICE_DRIVER_5_PRIVATE_DATA_FROM_DRIVER_BINDING (This);
...
{
  DriverBinding->Supported            = DeviceDriver5BindingSupported;
  DriverBinding->Start                = DeviceDriver5BindingStart;
  DriverBinding->Stop                 = DeviceDriver5BindingStop;
