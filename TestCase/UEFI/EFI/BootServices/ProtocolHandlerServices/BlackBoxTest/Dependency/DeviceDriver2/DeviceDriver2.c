...
} DEVICE_DRIVER_2_PRIVATE_DATA;

#define DEVICE_DRIVER_2_PRIVATE_DATA_FROM_DRIVER_BINDING(a) \
 _CR(a, DEVICE_DRIVER_2_PRIVATE_DATA, DriverBinding)

DEVICE_DRIVER_2_PRIVATE_DATA          *mPrivateData;

EFI_STATUS
InitializeDeviceDriver2 (
  IN EFI_HANDLE           ImageHandle,
  IN EFI_SYSTEM_TABLE     *SystemTable
  );

EFI_STATUS
DeviceDriver2BindingSupported (
...
//
EFI_DRIVER_ENTRY_POINT(InitializeDeviceDriver2)
  ...
  )
{
  ...
  Status = gtBS->AllocatePool (
                        EfiBootServicesData,
                        sizeof (DEVICE_DRIVER_2_PRIVATE_DATA),
                        (VOID**)&mPrivateData
                        );
  if (EFI_ERROR(Status)) {
    return Status;
  }
  gtBS->SetMem (mPrivateData,sizeof (DEVICE_DRIVER_2_PRIVATE_DATA),0);
  ...
  LoadedImageInfoPtr->Unload = DeviceDriver2Unload;
  ...
  )
{
  ...
  DEVICE_DRIVER_2_PRIVATE_DATA          *PrivateData;

  PrivateData = DEVICE_DRIVER_2_PRIVATE_DATA_FROM_DRIVER_BINDING (This);
...
{
  DriverBinding->Supported            = DeviceDriver2BindingSupported;
  DriverBinding->Start                = DeviceDriver2BindingStart;
  DriverBinding->Stop                 = DeviceDriver2BindingStop;
