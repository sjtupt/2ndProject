...
} DEVICE_DRIVER_18_PRIVATE_DATA;

#define DEVICE_DRIVER_18_PRIVATE_DATA_FROM_DRIVER_BINDING(a) \
 _CR(a, DEVICE_DRIVER_18_PRIVATE_DATA, DriverBinding)
 
DEVICE_DRIVER_18_PRIVATE_DATA          *mPrivateData;

EFI_STATUS
InitializeDeviceDriver18 (
  IN EFI_HANDLE           ImageHandle,
  IN EFI_SYSTEM_TABLE     *SystemTable
  );
  
EFI_STATUS
DeviceDriver18BindingSupported (
...
//
EFI_DRIVER_ENTRY_POINT(InitializeDeviceDriver18)
  ...
  )
{
  ...
  Status = gtBS->AllocatePool (
                        EfiBootServicesData,
                        sizeof (DEVICE_DRIVER_18_PRIVATE_DATA),
                        (VOID**)&mPrivateData
                        );
  if (EFI_ERROR(Status)) {
    return Status;
  }
  gtBS->SetMem (mPrivateData,sizeof (DEVICE_DRIVER_18_PRIVATE_DATA),0);
  ...
  LoadedImageInfoPtr->Unload = DeviceDriver18Unload;
  ...
  )
{
  ...
  DEVICE_DRIVER_18_PRIVATE_DATA          *PrivateData;
  
  PrivateData = DEVICE_DRIVER_18_PRIVATE_DATA_FROM_DRIVER_BINDING (This);
...
{
  DriverBinding->Supported            = DeviceDriver18BindingSupported;
  DriverBinding->Start                = DeviceDriver18BindingStart;
  DriverBinding->Stop                 = DeviceDriver18BindingStop;
